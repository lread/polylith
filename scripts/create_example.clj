...
(defn test-result->output
  ...
  [{:keys [out exit]}]
  (-> out
      (str/replace #"(Execution time: )[0-9]+" "$12")
      (str "\n# Exit code: " exit "\n")))
...
(defn component [{:keys [ws-dir fake-sha images-dir sections-dir output-dir]}]
  (let [poly (fn-default-opts sh/poly {:dir ws-dir})
    ...
    (copy [(fs/file sections-dir "component/workspace.edn")      ws-dir] ;; TODO: review with Joakim, this was only in create-example.sh
    ...
    (polyx (format "info fake-sha:%s out:%s" fake-sha (fs/file images-dir "component/info.png")))))
...
(defn polyx [{:keys [ws-dir fake-sha sections-dir images-dir] :as opts}]
  ...
  ;; TODO: review with Joakim, create-example.sh also runs these polyx commands, but oddly without fake-sha and outputs to different location
  (poly-infos opts "" "polyx-info.txt" "polyx/info.png")
  (sh/polyx {:dir ws-dir }
         (format "overview fake-sha:%s out:%s" fake-sha (fs/file images-dir "polyx/overview.png")))
  ;; restore deps.edn
  (copy [(fs/file sections-dir "project/deps.edn") ws-dir]))
...
(defn git [{:keys [ws-dir output-dir] :as opts}]
  (let [shell (fn-default-opts sh/shell {:dir ws-dir})
    ...
    (poly-infos opts "" "git-info.txt" "git/info.png")
    (shell "git log")
    (shell "git add --all")
...
(defn tagging [{:keys [ws-dir sections-dir fake-sha fake-sha2] :as opts}]
  (let [shell (fn-default-opts sh/shell {:dir ws-dir})
    ...
    (poly-infos opts "" "tagging-info-1.txt" "tagging/info-01.png")
    ...
    ;; TODO: Joakim, info-2 and info-3 are dupes, is that intentional?
    (poly-infos opts "since:release" "tagging-info-2.txt" "tagging/info-02.png")
    (poly-infos opts "since:release" "tagging-info-3.txt" "tagging/info-03.png")
    (poly-infos (assoc opts :fake-sha fake-sha)
                "since:previous-release" "tagging-info-4.txt" "tagging/info-04.png")
    (shell "git log --pretty=oneline")))

(defn flags [{:keys [fake-sha2] :as opts}]
  (let [opts (assoc opts :fake-sha fake-sha2)]
    (poly-infos opts ":resources" "flags-info.txt" "flags/info.png")))
...
(defn testing [{:keys [ws-dir fake-sha2 sections-dir output-dir] :as opts}]
  (let [opts (assoc opts :fake-sha fake-sha2)
    ...
    (poly-infos opts "" "testing-info-1.txt" "testing/info.png")
    ...
    (poly {:alter-out-fn test-result->output
          ...
          "test color-mode:none")
    (poly-infos opts ":dev"                  "testing-info-2.txt"  "testing/info-dev.png")
    (poly-infos opts "project:dev"           "testing-info-2b.txt" "testing/info-project-dev.png")
    (poly-infos opts "project:cl:dev"        "testing-info-3.txt"  "testing/info-project-cl-dev.png")
    (poly-infos opts ""                      "testing-info-3a.txt" "testing/info-filter-on-bricks.png")
    (poly-infos opts "brick:cli"             "testing-info-3b.txt" "testing/info-brick-cli.png")
    (poly-infos opts ":no-changes"           "testing-info-3c.txt" "testing/info-brick-cli-no-changes.png")
    (poly-infos opts "brick:cli :no-changes" "testing-info-3d.txt" "testing/info-brick-cli-no-changes-all.png")
    ...
    (poly-infos opts ""         "testing-info-4.txt" "testing/info-project-dir.png")
    (poly-infos opts ":project" "testing-info-5.txt" "testing/info-project.png")
    (shell "git add --all")
    (shell "git commit -m 'Added tests'")
    (shell "git tag -f stable-lisa")
    (poly-infos opts ""                 "testing-info-6.txt"  "testing/info-added-tests.png")
    (poly-infos opts ":all-bricks"      "testing-info-7.txt"  "testing/info-all-bricks.png")
    (poly-infos opts ":all-bricks :dev" "testing-info-8.txt"  "testing/info-all-bricks-dev.png")
    (poly-infos opts ":all"             "testing-info-9.txt"  "testing/info-all.png")
    (poly-infos opts ":all :dev"        "testing-info-10.txt" "testing/info-all-dev.png")
    ;; TODO: Joakim, why bother? this the the same as testing-info-10.txt, no?
...
(defn profile [{:keys [ws-dir sections-dir fake-sha2 output-dir] :as opts}]
  (let [shell (fn-default-opts sh/shell {:dir ws-dir})
    ...
    (poly-infos opts "+"       "profile-info-1.txt" "profile/info-all-aliases.png")
    (poly-infos opts "+remote" "profile-info-2.txt" "profile/info-with-remote-profile.png")
    ...
    (poly-infos opts ":loc" "profile-info-4.txt" "profile/info-loc.png")
    (poly {:alter-out-fn test-result->output
...
(defn main []
  (let [start-time-ms (System/currentTimeMillis)
        ...
        work-dir (fs/create-dirs (fs/file (fs/temp-dir) (str "polylith-" timestamp)))
