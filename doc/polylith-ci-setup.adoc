...
NOTE: This document talks specifically about how we've set up continuous integration for the https://github.com/polyfy/polylith[polylith repository].

We use https://circleci.com[CircleCI] for our continuous integration server, which coordinates validation, testing, and deployment workflows.
It:

* deploys artifacts to https://clojars.org/search?q=polylith[Clojars]
* triggers doc building on https://cljdoc.org/versions/polylith/clj-poly[cljdoc]
* and publishes releases to https://github.com/polyfy/polylith/releases[GitHub]

See our link:/.circleci/config.yml[CircleCI configuration file] for details.

[#releases]
== Releases

https://github.com/polyfy/polylith/issues[Issues] are addressed in https://github.com/polyfy/polylith/pulls[pull requests].
Pull requests are reviewed, approved, and then squashed to a commit to master.
Each commit to master triggers a release.

As you might have guessed, a `SNAPSHOT` https://github.com/polyfy/polylith/blob/15453ebc5a86f52ee1c75ec52ebb19da4b113c30/components/version/src/polylith/clj/core/version/interface.clj#L27[revisions] creates a snapshot release; all other revisions are considered final releases.
